 ...
 *)
fun get (env : environment) (x : string) : int option =
...
fun evalAexp (env : environment) :  aexp -> int = fn
  ...
  | Variable x => (
      case get env x of
        SOME n => n
      | NONE => raise Fail ("uninitialized variable " ^ x)
    )
...
fun runCommand (env : environment) : command -> environment = fn
    Assign (s, e) => set s (evalAexp env e) env
  | If (b, cmds1, cmds2) => if (evalBexp env b) then (runCommands env cmds1)
                            else (runCommands env cmds2)
  | Loop (cmds) => (runCommand (runCommands env cmds) (Loop (cmds))
                    handle BreakExn e => e 
                         | ContinueExn e => runCommand e (Loop (cmds)))
  | Break => raise (BreakExn (env))
  | Continue => raise (ContinueExn (env))
  | Return (exp) => raise (ReturnExn (evalAexp env exp))
